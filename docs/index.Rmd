...
-   Data by Prescriber Location from January 2022 to December 2024: <https://www.opendata.nhs.scot/dataset/prescriptions-in-the-community>
-   GP Practice Population Demographics January 2023, 2024 & 2025: <https://www.opendata.nhs.scot/dataset/gp-practice-populations>
-   GP Practice Contact Details and List Sizes January 2025: <https://www.opendata.nhs.scot/dataset/gp-practice-contact-details-and-list-sizes>
...
ded_seasonal_summary <- ded_prescriptions %>%
  mutate(month = paid_date_month %% 100, # Since the dates comes in the format YYYYMM, you can divide by 100 and take the remainder to find the month.
    ...
    month_name = month.name[month]) %>% # A DRY way to assign the actual name of the month based on the month's corresponding number from 1-12.
...
# These datasets are joined and show the number of DED prescriptions by GP practice each year.
...
# Weighted averages are plotted against the paid quantity per person of each practice. Again, a log scale is used.
